...
function isEquippedItem(item) {
  ...
  if (item.slot === "character") return state.settings.character === item.id;
  if (item.slot === "trail") return (state.settings.equippedTrail || "trail_none") === item.id;
  if (item.slot === "mapTheme") return (state.settings.mapTheme || "map_classic") === item.id;
...
function getSectionMeta(sectionId) {
  switch (sectionId) {
    case "characters":
      return { id: "characters", icon: "🧍", title: "Characters", subtitle: "Pick who leads the quest." };
    case "trails":
      return { id: "trails", icon: "✨", title: "Trails", subtitle: "Style your movement on the map." };
    case "themes":
      return { id: "themes", icon: "🗺️", title: "Themes", subtitle: "Change the feel of the world." };
    case "boosts":
      return { id: "boosts", icon: "⚡", title: "Boosts", subtitle: "Extra utility for tougher missions." };
    case "weapons":
      return { id: "weapons", icon: "🐝", title: "Weapons", subtitle: "Buy Bee Arrow ammo for Territory attacks." };
    case "owned":
      return { id: "owned", icon: "🎒", title: "Owned", subtitle: "Everything you already have." };
    default:
      return { id: sectionId, icon: "🛒", title: sectionId, subtitle: "" };
  }
}
...
export function equipShopItem(itemId) {
  ...
  if (slot === "character") {
    ...
    if (item.id === "char_chicken") window.playSound?.("chickenbuy.mp3");
    if (item.id === "char_ghost") window.playSound?.("ghost.mp3");
    if (item.id === "char_frog") window.playSound?.("frog.mp3");
