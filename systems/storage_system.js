export function createStorageSystem({ getDefaultAdaptiveProfile, normaliseAdaptiveProfile }) {
  ...
  function normaliseRoute(route = null) {
    ...
    return {
      ...
      completedNodes: Number.isFinite(route.completedNodes) ? route.completedNodes : 0,
      rebuildPoints: Number.isFinite(route.rebuildPoints) ? route.rebuildPoints : 0,
  ...
  function normaliseRebuild(rebuild = {}) {
    ...
    return {
      abbey: {
        ...
        completedRoutes: Array.isArray(abbey.completedRoutes) ? abbey.completedRoutes : [],
  ...
  function normaliseBossProgress(progress = {}) {
    ...
    Object.entries(progress || {}).forEach(([key, value]) => {
      ...
      out[key] = {
        ...
        failedAttempts: Number.isFinite(safe.failedAttempts) ? safe.failedAttempts : 0,
        wrongAnswers: Number.isFinite(safe.wrongAnswers) ? safe.wrongAnswers : 0,
        solved: !!safe.solved,
  ...
  function normaliseLoadedState(parsed) {
    ...
    return {
      ...
      unlockedMysteries: Array.isArray(safe.unlockedMysteries) ? safe.unlockedMysteries : [],
      ...
      quizProfiles:
        safe.quizProfiles && typeof safe.quizProfiles === "object"
          ? {
              ...
              adult: normaliseAdaptiveProfile(safe.quizProfiles.adult || {}, "adult"),
            }
          : {
              kid: getDefaultAdaptiveProfile("kid"),
              teen: getDefaultAdaptiveProfile("teen"),
      ...
      purchasedItems: Array.isArray(safe.purchasedItems) ? safe.purchasedItems : [],
      inventory: safe.inventory && typeof safe.inventory === "object" ? safe.inventory : {},
      captainNotes: Array.isArray(safe.captainNotes) ? safe.captainNotes : [],
      completedPins:
        safe.completedPins && typeof safe.completedPins === "object" ? safe.completedPins : {},
  ...
  function attach(BQ) {
    ...
    async function importSaveFileFromInput(file) {
      try {
        ...
        const migrated = migrateSave(parsed);
        BQ.state = normaliseLoadedState(migrated);
    ...
    function resetAllProgress() {
      try {
        ...
        BQ.state = structuredClone(DEFAULT_STATE);
        window.state = BQ.state;
        saveStateNow(true);
        BQ.renderEverything?.();
        return true;
    ...
    BQ.saveState = saveState;
    BQ.saveStateNow = saveStateNow;
    BQ.importSaveFileFromInput = importSaveFileFromInput;
    BQ.exportSaveFile = exportSaveFile;
    BQ.resetAllProgress = resetAllProgress;
    ...
    window.BQStorage = {
      ...
      getState: () => structuredClone(BQ.state),
    };
  }
