...
async function tagSpecificRunner(runner) {
  ...
  const hunter = getLocalPlayer?.();

  if (!hunter) {
    speakText?.("No hunter found.");
    return false;
  }
  ...
  if (hunter.status !== "free") {
    speakText?.("You cannot tag right now.");
    return false;
  }

  if (hunter.carryingRunnerId) {
    speakText?.("Take your captured runner to jail first.");
    return false;
  }

  if (!runner || runner.role !== "runner") {
    speakText?.("No valid runner selected.");
    return false;
  }

  if (runner.status !== "free") {
    speakText?.("That runner is not free.");
    return false;
  }

  if (!hunter.position || !runner.position) {
    speakText?.("GPS position is not ready.");
    return false;
  }
  ...
  if (distance > tagRadius) {
    speakText?.("Runner is too far away.");
    return false;
  }

  runner.status = "captured";
  runner.escortedBy = hunter.id;
  runner.capturedAt = Date.now();

  hunter.carryingRunnerId = runner.id;

  playLeoidsSound?.("hunter_tag", 1);
  ...
  showLeoidsCinematicOverlay?.({
    title: "RUNNER CAUGHT",
    subtitle: `Take ${runner.name || "the runner"} to jail.`,
    icon: "🔴",
    theme: "hunter",
    duration: 1700,
  });
  ...
  showLeoidsEvent?.(
    "RUNNER CAUGHT",
    `Hunter must take ${runner.name || "runner"} to jail before tagging again.`,
    "🔴",
    "hunter"
  );

  speakText?.("Runner caught. Take them to jail.");

  await syncPlayerToOnline?.(runner);
  await syncPlayerToOnline?.(hunter);
  await loadOnlinePlayers?.();
...
async function tagNearestRunner() {
  ...
  if (!leoidsState.huntersReleased) {
    ...
    speakText?.("Wait for hunter release.");
    return false;
  }

  if (!local.position) {
    speakText?.("Your GPS position is not ready.");
    return false;
  }
  ...
  if (!nearestRunner || nearestDistance > tagRadius) {
    showLeoidsEvent?.(
      "NO TAG",
      `No runner inside tag range.\nClosest: ${
        Number.isFinite(nearestDistance) ? `${Math.round(nearestDistance)}m` : "--"
      }\nRange: ${tagRadius}m`,
      "❌",
      "danger"
    );
  ...
  return await tagSpecificRunner(nearestRunner.id);
}


async function checkHunterEscortDelivery() {
  if (!leoidsState.active) return false;
  if (!leoidsState.basePoint) return false;
  ...
  for (const hunter of hunters) {
    const runner = leoidsState.players.find(
      (player) => player.id === hunter.carryingRunnerId
    );

    if (!runner) {
      hunter.carryingRunnerId = null;
      continue;
    }

    const baseRadius = Number(leoidsState.baseRadius || DEFAULT_BASE_RADIUS);
    const distanceToBase = distanceMeters(hunter.position, leoidsState.basePoint);

    if (distanceToBase > baseRadius) continue;

    runner.status = "jailed";
    runner.jailedAtBase = true;
    runner.escortedBy = null;

    hunter.carryingRunnerId = null;
    hunter.score = Number(hunter.score || 0) + 100;
    hunter.coins = Number(hunter.coins || 0) + 10;
    ...
    showLeoidsCinematicOverlay?.({
      title: "RUNNER JAILED",
      subtitle: `${runner.name || "Runner"} delivered to jail.`,
      icon: "🔴",
      theme: "hunter",
      duration: 1500,
    });
    ...
    showLeoidsEvent?.(
      "RUNNER JAILED",
      `${runner.name || "Runner"} has been delivered to jail.`,
      "🔴",
      "hunter"
    );

    speakText?.("Runner delivered to jail.");
...
function updateLeoidsLiveActionButton() {
  ...
  if (local.role === "hunter") {
    ...
    if (local.carryingRunnerId) {
      btn.style.display = "block";
      btn.classList.remove("urgent");
      btn.innerText = "🔴 TAKE RUNNER TO JAIL";
      btn.style.background = "#ff3b3b";
      btn.style.color = "white";
      btn.style.boxShadow = "0 0 38px rgba(255,59,59,.75)";
      btn.onclick = () => {
        speakText?.("Take the runner back to jail before tagging again.");
      };
      return;
    }
...
function tickRound() {
  ...
  leoidsState.timeLeft = Math.max(0, Number(leoidsState.timeLeft || 0) - 1);

  if (!leoidsState.huntersReleased) {
    leoidsState.hunterDelayLeft = Math.max(
      0,
      Number(leoidsState.hunterDelayLeft || 0) - 1
    );
    ...
    if (
      ...
      playLeoidsSound?.(
        leoidsState.hunterDelayLeft <= 5 ? "countdown_final" : "countdown_tick",
        leoidsState.hunterDelayLeft <= 5 ? 0.9 : 0.45
      );
    ...
    if (leoidsState.hunterDelayLeft <= 0) {
      ...
      if (local?.role === "hunter") {
        ...
        showLeoidsCinematicOverlay?.({
          title: "HUNTERS RELEASED",
          subtitle: "Run. Hide. Rescue your team.",
          icon: "🏃",
          theme: "runner",
          duration: 1700,
        });
...
async function endOnlineRound(reason = "manual") {
  const supabase = getSupabaseSafe?.();
  ...
  if (typeof updateOnlineSession === "function") {
    return await updateOnlineSession(payload);
  }
  ...
  if (supabase.client) {
    ...
    if (error) {
      console.warn("Could not end online round:", error);
      return null;
    }
...
async function endRound(reason = "manual", options = {}) {
  ...
  if (
    ...
    typeof endOnlineRound === "function"
  ) {
...
async function openOnlineLobbyScreen(sessionId = leoidsState.onlineSessionId, options = {}) {
  const autoStartGps = !!options.autoStartGps;
  const supabase = window.LEOIDSSupabase;
  ...
  async function refreshLobbyScreen() {
    ...
    try {
      if (typeof supabase.getSession === "function") {
        session = await supabase.getSession(activeSessionId);
      }

      if (typeof supabase.loadPlayers === "function") {
        players = await supabase.loadPlayers();
      }
    } catch (err) {
      console.warn("Could not refresh lobby:", err);
    }
            ...
            .map((player) => {
              ...
              return `
                <div style="
                  ...
                  margin-top:8px;
                ">
                  <span>${icon} ${player.display_name || player.name || "Player"}</span>
  ...
  document
    .getElementById("btn-leoids-lobby-close")
    ?.addEventListener("click", closeLobbyScreen);

  document
    .getElementById("btn-leoids-lobby-gps")
    ?.addEventListener("click", () => {
      openGameMapFromLobby({ startLocation: true });
    });

  document
    .getElementById("btn-leoids-lobby-help")
    ?.addEventListener("click", () => {
  ...
  document
    .getElementById("btn-leoids-lobby-host-setup")
    ?.addEventListener("click", async () => {
      const session = await supabase.getSession?.(leoidsState.onlineSessionId);
  ...
  document
    .getElementById("btn-leoids-lobby-end-session")
    ?.addEventListener("click", async () => {
      const session = await supabase.getSession?.(leoidsState.onlineSessionId);
  ...
  document
    .getElementById("btn-leoids-lobby-runner")
    ?.addEventListener("click", async () => {
      ...
      await supabase.joinSession({
        ...
        displayName:
          supabase.playerName || leoidsState.onlinePlayerName || "Player",
        role: "runner",
      });
  ...
  document
    .getElementById("btn-leoids-lobby-hunter")
    ?.addEventListener("click", async () => {
...
async function openOnlineSessionBrowser() {
  ...
  const joinSessionSafely = async ({ sessionId, displayName, role }) => {
    ...
    if (window.LEOIDS && typeof window.LEOIDS.joinOnlineSession === "function") {
      return await window.LEOIDS.joinOnlineSession({
        sessionId,
        displayName,
        role,
      });
    ...
    return await supabase.joinSession({
      sessionId,
      displayName,
      role,
    });
  ...
  const isMine = (session) => {
    const myName = leoidsState.onlinePlayerName || supabase.playerName || "";
    const hostName = session?.host_name || "";

    return !!myName && !!hostName && myName.trim() === hostName.trim();
  };

  const openNameRolePicker = ({
    title = "JOIN LOBBY",
    defaultName = "Kyle",
    onConfirm,
  }) => {
    ...
    picker.style.background = "rgba(0,0,0,.9)";
    ...
    picker.innerHTML = `
      <div style="
        ...
        font-family:system-ui,-apple-system,BlinkMacSystemFont,'Segoe UI',sans-serif;
      ">
        <h2 style="margin:0;color:#00d4ff;text-align:center;">${title}</h2>
        ...
        <button id="leoids-picker-spectator" type="button" style="
          width:100%;
          min-height:46px;
          margin-top:10px;
          border-radius:16px;
    ...
    document
      .getElementById("leoids-picker-runner")
      ?.addEventListener("click", () => submit("runner"));

    document
      .getElementById("leoids-picker-hunter")
      ?.addEventListener("click", () => submit("hunter"));

    document
      .getElementById("leoids-picker-spectator")
      ?.addEventListener("click", () => submit("spectator"));

    document
      .getElementById("leoids-picker-cancel")
  ...
  try {
    sessions = await supabase.listPublicSessions();
  } catch (err) {
    console.warn("Could not load public sessions:", err);
    sessions = [];
  }
        ...
        .map((session) => {
          ...
          return `
            <div style="
              ...
              color:white;
              box-shadow:0 0 20px rgba(0,212,255,.13);
            ">
              <div style="font-size:17px;color:#00d4ff;font-weight:1000;">
  ...
  modal.innerHTML = `
    ...
    ">
      <h2 style="margin:0;color:#00d4ff;text-align:center;">
        ONLINE LEOIDS LOBBIES
      </h2>

      <p style="opacity:.82;margin:8px 0 14px;text-align:center;">
...
function getLeoidsHudStatusText() {
  ...
  if (!local) return "JOINING";

  if (local.role === "spectator") return "SPECTATOR";

  if (!leoidsState.active) return "WAITING";

  if (local.status === "jailed") return "JAILED";

  if (local.carryingRunnerId || local.escortTargetId) {
    return "DELIVER";
  }
  ...
  if (local.role === "hunter" && local.position) {
    ...
    if (closestDistance <= tagRadius) return "TAG READY";
    if (closestDistance <= tagRadius * 2) return `${Math.round(closestDistance)}M`;
...
function setRole(role = "runner") {
  ...
  if (local) {
    local.role = safeRole;
    local.status = safeRole === "spectator" ? "watching" : "free";
    local.jailedAtBase = false;
    local.carryingRunnerId = null;
    local.escortTargetId = null;
  }
  ...
  if (safeRole === "hunter") speakText?.("Hunter selected.");
  else if (safeRole === "runner") speakText?.("Runner selected.");
  else speakText?.("Spectator mode.");
}
