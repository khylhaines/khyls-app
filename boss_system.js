...
export function createBossSystem({
  ...
  function getBossDefsSafe() {
    return typeof getBossDefs === "function" ? getBossDefs() || {} : {};
  }

  function getBossModePinIdsSafe() {
    return typeof getBossModePinIds === "function"
      ? getBossModePinIds() || []
      : [];
  }

  function getBossDef(pinId) {
    const defs = getBossDefsSafe();
    return defs[pinId] || null;
  }

  function ensureBossProgressStore() {
    const s = state();
    if (!s.bossProgress || typeof s.bossProgress !== "object") {
      s.bossProgress = {};
    }
    return s.bossProgress;
  }
  ...
  function isBossPin(pin) {
    return !!pin?.id && getBossModePinIdsSafe().includes(pin.id);
  }
  ...
  function applyBossPenalty(def) {
    ...
    s.meta.xp = Math.max(
      0,
      Number(s.meta.xp || 0) - Math.abs(Number(penalty.xp || 0))
    );
  }
  ...
  function runBossStep() {
    ...
    const { pinId, steps, stepIndex } = task.boss;
  ...
  function answerBossOption(index) {
    ...
    const { pinId, steps, stepIndex, def } = task.boss;
    const step = steps[stepIndex];
    const feedback = $("task-feedback");
    const progress = getBossProgress(pinId);
  ...
  function finishBossMission() {
    ...
    s.meta.xp = Number(s.meta.xp || 0) + bonus.xp;
    s.meta.tokens = Number(s.meta.tokens || 0) + bonus.tokens;
